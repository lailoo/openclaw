...
- Plugins/Install: sanitize `workspace:` protocol entries from all dependency fields (not just devDependencies) before npm install, preventing `EUNSUPPORTEDPROTOCOL` failures when installing published plugins like `@openclaw/feishu`. (#24578) Thanks @zhuangjinjin.
